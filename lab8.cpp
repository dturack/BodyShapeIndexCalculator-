...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 2) {
        ...
        for (int i=0; i<81; i++) {
            ...
            cout << dataArr[i*5+2];
            cout << setw(11);
            cout << setprecision(7) << fixed << dataArr[i*5+4];
            cout.unsetf(ios_base::fixed);
            sbsi2.h = dataArr[i*5+0];
            sbsi2.wc = dataArr[i*5+1];
            sbsi2.bsa = dataArr[i*5+2];
            sbsi2.vtc = dataArr[i*5+3];
            cout << setw(11);
            cout << setprecision(7) << fixed << bodyShapeIndex(&sbsi2);
            cout.unsetf(ios_base::fixed);
            diff = bodyShapeIndex(&sbsi2)-dataArr[i*5+4];
            diff = fabs(diff);
            if (diff > tolerance){
        ...
        cout << "bad element count: " << bec << endl;
